...
func GetProductDetailsBySlug(c *gin.Context) {
	...
	product := services.FetchProductsDetails(&models.Product{Slug: productSlug}, true)
	if product.ID == 0 {
...
func CreateProduct(c *gin.Context) {
	user := c.Keys["curremtUser"].(models.User)
	if user.IsNotAdmin() {
	...
	for k, v := range form.Value {
		if strings.HasPrefix(k, "tags[") {
			...
			name := result[1]
			description := v[0]
			database.Where(&models.Tag{Slug: slug.Make(name)}).Attrs(models.Tag{Name: name, Description: description}).FirstOrCreate(&tag)
			tags[tagPtr] = tag
			tagPtr++
		}
		if strings.HasPrefix(k, "category[") {
			...
			name := result[1]
			description := v[0]
			database.Where(&models.Category{Slug: slug.Make(name)}).Attrs(models.Category{Name: name, Description: description}).FirstOrCreate(&category)
...
func ProductDelete(c *gin.Context) {
	if services.DeleteProduct(&models.Product{Slug: c.Param("slug")}) != nil {
		c.JSON(http.StatusNotFound, dtos.CreateDetailedErrorDto("products", errors.New("Invalid slug")))
		return
	}
